...
class SOAPSummarizer:
    ...
    async def summarize(self, soap:str, expected_token_num:int, session, thread_uuid):
        messages = [{"role":"system","content": self._system_content},
        ...
        or (not 'message' in completion['choices'][0]):
            print(content_error_message + " スレッドID:" + str(thread_uuid))
            print(f"Failed to get response from OpenAI. response={completion}" + " スレッドID:" + str(thread_uuid))
        ...
        if ('finish_reason' in completion['choices'][0]) and (completion['choices'][0]['finish_reason'] == "content_filter"):
            ...
            print(errmsg + " スレッドID:" + str(thread_uuid))
